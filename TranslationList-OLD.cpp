...
Piece TranslationList::iterator::operator*()
{
	...
	if(last)
	{
		std::cerr<<"TranslationList.cpp:Piece& TranslationList::iterator::operator*():Error: "
...
TranslationList::iterator::iterator(iterator& rhs):translator(rhs.translator)
{
	starter = rhs.starter;
	last = rhs.last;
}
...
TranslationList::iterator& TranslationList::iterator::operator++()
{
	if(last)
	{
		std::cerr<<"TranslationList.cpp:TranslationList::iterator& TranslationList::iterator::"
			<<"operator++():Error: Iteration past the end of the list!"<<std::endl;
		exit(1);
	}
	int dimension=1;
	//The dimension whose units are moving is the lowest one that's not on its last translation
	while(dimension <= translator.GetMaxDims() && 
...
bool TranslationList::iterator::operator!=(TranslationList::iterator& rhs)
{
